...
var Wires = {};
 ...
 Wires.run = function(wire_name) {
    if( typeof Wires[wire_name] === 'function') {
        Wires[wire_name] = Wires[wire_name]();
    }
...
for(var i = 0; i < grouped_instructions.length; i++) {
    var instruction_string = grouped_instructions[i].join(" ");
    var split_instructions = instruction_string.split(" -> ");


    Wires[split_instructions[1]] = new Function( 'return ' + split_instructions[0].replace(/(AND|OR|RSHIFT|LSHIFT|NOT)/, function swapOps(op) {
        return {
            AND: '&',
            OR: '|',
            LSHIFT: '<<',
...
function groupInstructions() {
    while(unresovled_instructions_left) {
        if (ungrouped_instructions[ungroup_index + 1] === '->') {
            for(var i = 0; i < 3; i++) {
                temp_array = temp_array.concat(ungrouped_instructions[ungroup_index + i]);
            }
            ungroup_index += 3;
        }
        else if (ungrouped_instructions[ungroup_index] === "NOT") {
            for(var i = 0; i < 4; i++) {
                temp_array = temp_array.concat(ungrouped_instructions[ungroup_index + i]);
            }
            ungroup_index += 4;
