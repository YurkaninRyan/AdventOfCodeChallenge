...
var ungrouped_instructions = input.split(" ");
var grouped_instructions = [];
var unresovled_instructions_left = true;
var group_index = 0;
var ungroup_index = 0;
var temp_array = [];
var light_array_one = [];
...
for(var j = 0; j < grouped_instructions.length; j++) {
    var command = grouped_instructions[j][0];
    var from = grouped_instructions[j][1].split(",");
    var to   = grouped_instructions[j][3].split(",");

    for(var ii = parseInt(from[0]); ii <= parseInt(to[0]); ii++) {
        for(var jj = parseInt(from[1]); jj <= parseInt(to[1]); jj++) {
            switch (command) {
                case "turnon":
                    light_array_one[ii][jj] = true;
                    light_array_two[ii][jj] += 1;
                    break;
                case "turnoff":
                    light_array_one[ii][jj] = false;
                    if(light_array_two[ii][jj] !== 0) { light_array_two[ii][jj] -= 1; }
                    break;
                case "toggle":
                    light_array_one[ii][jj] = !light_array_one[ii][jj];
                    light_array_two[ii][jj] += 2;
                    break;
                default:
                    console.log("Bad instruction " + command);
                }
            }
    }
...
countLightsProblemOne();
countLightsProblemTwo();


// Group the split instruction list into strings of instructions.
function groupInstructions() {
    while(unresovled_instructions_left) {
        if(ungrouped_instructions[ungroup_index] === "turn") {
            for(var i = 0; i < 5; i++) {
                if (i === 0) {
                    temp_array[0] = ungrouped_instructions[ungroup_index] + ungrouped_instructions[ungroup_index + 1];
                    continue;
                }
                if (i === 1) { continue; }
                temp_array = temp_array.concat(ungrouped_instructions[ungroup_index + i]);
            }

            grouped_instructions[group_index] = temp_array;
            group_index++;
            ungroup_index += 5;
        }

        else if (ungrouped_instructions[ungroup_index] === "toggle") {
            for(var i = 0; i < 4; i++) {
                temp_array = temp_array.concat(ungrouped_instructions[ungroup_index + i]);
            }
        ...
        temp_array = [];
        if(ungroup_index === ungrouped_instructions.length) { break; }
    }
}
...
function initLightField() {
    for( var i = 0; i <= 999; i++ ) {
        ...
        light_array_two[i] = [];
        for( var j = 0; j <= 999; j++) {
            light_array_one[i][j] = false;
            light_array_two[i][j] = 0;
        }
    }
}

function countLightsProblemOne() {
    for( var m = 0; m <= 999; m++ ) {
        for( var n = 0; n <= 999; n++) {
            if(light_array_one[m][n]) { on_count += 1; }
        }
    }

    console.log("The amount of lights turned on is " + on_count);
}

function countLightsProblemTwo() {
    for( var m = 0; m <= 999; m++ ) {
        for( var n = 0; n <= 999; n++) {
            brightness_count += light_array_two[m][n];
        }
    }

    console.log("The total brightness of the lights is " + brightness_count);
}
